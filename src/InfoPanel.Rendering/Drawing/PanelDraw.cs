...
namespace InfoPanel.Drawing
{
    ...
    internal class PanelDraw
    {
        ...
        public static void Run(Profile profile, SkiaGraphics g, bool preview = false, float scale = 1, bool cache = true, string cacheHint = "default", FpsCounter? fpsCounter = null)
        {
            ...
            {
                if (RenderContext.ShowGridLines)
                {
                    var gridSpace = RenderContext.GridLinesSpacing;
                    var gridColor = RenderContext.GridLinesColor;
                ...
                if (SKColor.TryParse(RenderContext.SelectedItemColor, out var color))
                {
                ...
                {
                    Logger.Warning("Failed to parse selected item color: {Color}", RenderContext.SelectedItemColor);
                }
            }
