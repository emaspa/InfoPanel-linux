    ...
    public sealed class WebServerTask : BackgroundTask
    {
        ...
        protected async override Task DoWorkAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                _webApplication.Urls.Add($"http://{DeviceRuntime.Settings.WebServerListenIp}:{DeviceRuntime.Settings.WebServerListenPort}");
                ...
                _webApplication.MapGet("/", async context =>
                {
                    ...
                    if (DeviceRuntime.GetProfiles().Count == 0)
                    {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < DeviceRuntime.GetProfiles().Count; i++)
                        {
                            var profile = DeviceRuntime.GetProfiles()[i];
                            var profileUrl = $"http://{DeviceRuntime.Settings.WebServerListenIp}:{DeviceRuntime.Settings.WebServerListenPort}/{i}";
                ...
                _webApplication.MapGet("/{id}", async context =>
                {
                    ...
                    content = content.Replace("{{REFRESH_RATE}}", $"{DeviceRuntime.Settings.WebServerRefreshRate}");
                ...
                {
                    if (int.TryParse(context.Request.RouteValues["id"]?.ToString(), out int id) && id < DeviceRuntime.GetProfiles().Count)
                    {
